...
import ConversionBanner from "@/app/components/ConversionBanner";

const contentPath = "/posts/news";

const getPost = cache(
  async (contentPath: string, locale: string, slug: string) =>
    await getPostNotCached(contentPath, locale, slug),
);
    ...
    <article className="prose container mx-auto px-4 py-8 max-w-4xl">
      ...
      <div className="mb-8">
        ...
        <div className="flex items-center gap-4 text-gray-600 mb-8">
          ...
          <span>•</span>
          <span>
            {t("views", { count: postContent.frontmatter.viewCount as number })}
          </span>
        </div>
      </div>

      {/* Article content */}
      <div className="prose prose-lg max-w-none">{postContent.content}</div>
      <ConversionBanner />
    </article>
  );
}
